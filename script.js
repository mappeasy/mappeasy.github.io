...
async function queryFeaturesAndGetGeoJson(initialUnitId, stopPoint) {
    ...
    const gravityMain_layer = "https://geogimstest.houstontx.gov/arcgis/rest/services/HW/WastewaterUtilities/MapServer/25"
    const forceMain_layer = "https://geogimstest.houstontx.gov/arcgis/rest/services/HW/WastewaterUtilities/MapServer/24"
...
async function findNearestFeatureUnitId(longitude, latitude) {
    ...
    while (distance <= maxDistance) {
        const queryUrl = `https://geogimstest.houstontx.gov/arcgis/rest/services/HW/WastewaterUtilities/MapServer/18/query?geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&distance=${distance}&outFields=UNITID&geometry={"x":${longitude},"y":${latitude}}&units=esriSRUnit_Foot&f=json`;
...
async function findLSWWTP(assetType, unitID) {
    ...
    if([98,15,25].includes(assetType)){
         ...
         if (data.features.length > 0) {
             ...
             queryUrl = `https://geogimstest.houstontx.gov/arcgis/rest/services/HW/WastewaterUtilities/MapServer/17/query?where=REFERENCEID='${refID}'&outFields=${outfield}&f=pjson&outSR=4326`;
         } else {
            return null;
         }
    }
